...
public class DataSheetGraph extends JPanel implements DataSheetChangeListener {
    ...
    private double minX() {
        int size;
        if ( ( size = dataSheet.size()) <= 0 ) return 0;
        double result = dataSheet.get(0).getX();
        if (dataSheet != null) {
            for (int i = 0; i < size; i++)
                if (dataSheet.get(i).getX() < result)
                    result = dataSheet.get(i).getX();
        }
        return result;
    }
    private double maxX() {
    ...
    private double minY() {
        int size;
        if ( ( size = dataSheet.size()) <= 0 ) return 0;
        double result = dataSheet.get(0).getY();
        if (dataSheet != null) {
            for (int i = 1; i < size; i++)
                if (dataSheet.get(i).getY() < result)
                    result = dataSheet.get(i).getY();
        }
        return result;
    }
    private double maxY() {
    ...
    public void showGraph(Graphics2D gr) {
        ...
        if (dataSheet != null) {
            if (!isConnected) {
                ...
                double xOld = x0 + dataSheet.get(0).getX() * xScale;
                double yOld = y0 - dataSheet.get(0).getY() * yScale;
                for (int i = 1; i < dataSheet.size(); i++) {
                    double x = x0 + dataSheet.get(i).getX() * xScale;
                    double y = y0 - dataSheet.get(i).getY() * yScale;
